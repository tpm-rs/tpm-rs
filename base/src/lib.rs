...
/// TpmNvExp represents the NV index attributes (TPMA_NV_EXP).
/// See definition in Part 2: Structures, section 13.8.
#[repr(transparent)]
#[derive(Clone, Copy, PartialEq, Debug, Default, Marshalable)]
...
bitflags! {
    impl TpmaNvExp : u64 {
        ...
        /// Whether nv_read_lock may be used to set READLOCKED for this index.
        const READ_STCLEAR = 1 << 31;
	/// Whether external NV index contents are encrypted.
	const EXTERNAL_NV_ENCRYPTION = 1 << 32;
	/// Whether external NV index contents are integrity-protected.
	const EXTERNAL_NV_INTEGRITY = 1 << 33;
	/// Whether external NV index contents are rollback-protected.
...
impl TpmaNvExp {
    ...
    /// Returns the attribute for an index type (with all other field clear).
    const fn from_index_type(index_type: TpmNt) -> TpmaNvExp {
