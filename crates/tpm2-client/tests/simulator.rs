...
///  `cd crates/tpm2-client && docker compose run simulator_tests`
///
...
const DEFAULT_SIMULATOR_PROGRAM: &str = "target/debug/tpm2-simulator";
...
fn get_simulator_path() -> String {
    if let Ok(s) = std::env::var(ENV_VAR_SIMULATOR_PROGRAM) {
        return s;
    }
    ...
    let path = PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .parent()
        .unwrap()
        .join(DEFAULT_SIMULATOR_PROGRAM);
